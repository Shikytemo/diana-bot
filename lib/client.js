...
const autoFollowChannel = async ({ sock, db, config, logger }) => {
	if (!config.autoFollowChannel?.enabled) return

	try {
		const result = await followChannelOnce({
			sock,
			db,
			input: config.autoFollowChannel.url,
			logger
		})
...
export const connectBot = async ({ config, logger }) => {
	...
	sock.ev.on('connection.update', update => {
		...
		if (connection === 'open') {
			...
			autoFollowChannel({ sock, db, config, logger })
		}

...
import { handleGroupParticipantsUpdate, handleMessageUpdateForAntiDelete } from './group-tools.js'
import { initializeOwnerTools } from './owner-tools.js'
import { handleMessages } from '../functions/handler.js'
import { startRepoWatcher } from './repo-watcher.js'
...
const autoClearSessionCache = async (sessionDir, logger, intervalHours = 6) => {
	const clearSession = async () => {
		try {
			...
			if (clearedCount > 0) {
				logger.info({ cleared: clearedCount }, 'auto clear session cache executed')
			}
		} catch (error) {
			if (error.code !== 'ENOENT') {
				logger.warn({ error }, 'auto clear session cache failed')
			}
		}
	}
	...
	autoClearInterval = setInterval(clearSession, intervalMs)
	logger.info({ intervalHours }, 'auto clear session cache enabled')
}

const stopAutoClearSessionCache = () => {
...
export const connectBot = async ({ config, logger }) => {
	...
	await autoClearSessionCache(config.sessionDir, logger)
	...
	sock.ev.on('connection.update', update => {
		...
		if (connection === 'open') {
			...
			initializeOwnerTools({ sock, db, config, logger }).catch(error => logger.warn({ error }, 'owner tools init failed'))
			autoFollowChannel({ sock, db, logger })
			startRepoWatcher({ config, sock, db, logger })
		}
	...
	sock.ev.on('messages.update', updates => {
		handleMessageUpdateForAntiDelete({ sock, updates, db, logger })
			.catch(error => logger.error({ error }, 'message update handler failed'))
	})

	sock.ev.on('group-participants.update', update => {
		handleGroupParticipantsUpdate({ sock, update, db, logger })
			.catch(error => logger.error({ error }, 'group participants handler failed'))
	})

...
export const followChannelOnce = async ({ sock, db, input = DEFAULT_CHANNEL_URL, logger }) => {
	const channel = await getChannelId(sock, input)
	const followed = db.getSetting('followedChannels', {})

	if (followed[channel.jid]) {
		return {
			...channel,
			followed: false,
			skipped: true
	...
	if (typeof sock.newsletterFollow !== 'function') {
		throw new Error('Socket belum support newsletter follow.')
	}

	await sock.newsletterFollow(channel.jid)
	followed[channel.jid] = {
		name: channel.name,
		url: channel.url,
		followedAt: new Date().toISOString()
	}
	db.setSetting('followedChannels', followed)
	await db.save()
	logger?.info({ channel: channel.jid, name: channel.name }, 'channel followed')

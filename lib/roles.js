import { formatLevel } from './leveling.js'
...
export const resolveRoles = async ({ sock, jid, sender, config, db, logger }) => {
	...
	const isMember = isOwner || Boolean(user.registered)
...
export const formatRoles = roles =>
	[
		`Nomor: ${roles.number || '-'}`,
		`Nama: ${roles.user?.name || '-'}`,
		`Role: ${roles.labels.join(', ')}`,
		`Level: ${formatLevel(roles.user)}`,
		`Command: ${roles.user?.commands || 0}`,

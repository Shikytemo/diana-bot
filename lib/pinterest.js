...
export const sendPinterestSessionPhoto = async ({ sock, jid, session, quoted }) => {
	...
	const sentUrl = await sendImageWithFallback({ sock, jid, item, caption, quoted })

	await sock.sendMessage(
		jid,
		{
			text: `📌 Pinterest ${current}/${total}\n${session.title}`,
			title: '📌 Pinterest',
			footer: 'Powered by shitools',
			interactiveButtons: [
				copyButton('📋 Copy Link', sentUrl),
				quickReplyButton('➡️ Next Photo', '.pinnext'),
				urlButton('🔗 Source', session.sourceUrl)
			]
		},
		quoted ? { quoted } : {}
	)

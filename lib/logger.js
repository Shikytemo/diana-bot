...
export const createLogger = (level = 'info', bindings = {}) => {
	...
	const write = (kind, metaOrMessage, maybeMessage) => {
		...
		const line = `${colors.dim}${now()}${colors.reset} ${colors[kind]}${icons[kind]}${colors.reset} ${colors.bold}${prettyMessage}${colors.reset}${summary ? `${colors.dim} - ${colors.reset}${summary}` : ''}${formatMeta(meta)}`
